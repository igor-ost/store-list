...
export default function WorkLogForOrderTable({ logs, loading = false }: Props) {
  ...
  if (logs.length === 0) {
    return (
      <Card className="p-8 text-center">
        <p className="text-muted-foreground">Записей в журнале не найдено</p>
      </Card>
    )
  }
        ...
        <table className="w-full">
          <thead className="bg-muted border-b border-border">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-sm font-semibold text-foreground">Время добавления</th>
            </tr>
          </thead>
