...
const PERSONNEL_ROLES = ["manager", "technologist", "accountant", "admin"]
const WORKER_ROLES = ["seamstress", "cutter"]

export function WorkersList({ workers }: WorkersListProps) {
  const [workersList, setWorkers] = useState<WorkersGeneral[]>([])
  const [searchTerm, setSearchTerm] = useState("")
  const [currentPage, setCurrentPage] = useState(1)
  const [activeTab, setActiveTab] = useState<"personnel" | "worker">("personnel")
  ...
  const getFilteredByType = (list: WorkersGeneral[], type: "personnel" | "worker") => {
    const rolesList = type === "personnel" ? PERSONNEL_ROLES : WORKER_ROLES
    return list.filter((w) => {
      const normalizedRole = typeof w.role === "string" ? w.role.toLowerCase() : w.role
      return rolesList.some((r) => r.toLowerCase() === normalizedRole)
    })
  }

  const typeFilteredWorkers = getFilteredByType(workersList, activeTab)

  const filteredOrders = typeFilteredWorkers.filter((workers) => {
    const matchesSearch =
  ...
  const handleTabChange = (tab: "personnel" | "worker") => {
    setActiveTab(tab)
    setCurrentPage(1)
  }
